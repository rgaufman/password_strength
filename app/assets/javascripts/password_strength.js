(function(){
  ...
  // The number of characters, counted the way a person counts them: a codepoint
  // outside the basic plane is one character, not two.
  function characterCount(text) {
    var count = 0;

    for (var _character of text || "") {
      count++;
    }
  ...
  PasswordStrength.fn.test = function() {
    ...
    var reason = this.rejectionReason();
  ...
  PasswordStrength.fn.scoreFor = function(name) {
    ...
    switch (name) {
      case "password_size":
        if (this.password.length < SHORT_PASSWORD_SIZE) {
          score = -100;
        } else {
  ...
  PasswordStrength.fn.isTooShort = function() {
    ...
    return characterCount(this.password) < this.minLength;
  };

  // The reason to reject the password outright, or null when it is worth
  // scoring. Mirrors PasswordStrength::Base#rejection_reason.
  PasswordStrength.fn.rejectionReason = function() {
    if (this.isTooShort()) {
      return "too_short";
    }
    ...
    if (this.usesCommonWord()) {
      return "common_word";
    }

    if (this.containInvalidRepetition()) {
  ...
  PasswordStrength.fn.usesCommonWord = function() {
    var list = blocklistSet(PasswordStrength.blocklist || PasswordStrength.commonWords);
    var candidates = this.blocklistCandidates(this.password);

    for (var i = 0; i < candidates.length; i++) {
      if (list.has(candidates[i])) {
        return true;
      }
    }
  ...
  PasswordStrength.fn.blocklistCandidates = function(text) {
    ...
    if (normalised.normalize && NON_ASCII_RE.test(normalised)) {
      normalised = normalised.normalize("NFKC");
    }

    normalised = normalised.toLowerCase();

    var leet = normalised.replace(LEET_RE, function(character) {
      return LEET_SUBSTITUTIONS[character];
    });
  ...
  PasswordStrength.fn.sequences = function(text) {
    ...
    for (var character of text || "") {
      currentCode = character.codePointAt(0);
  ...
  PasswordStrength.commonWords = COMMON_WORDS;
