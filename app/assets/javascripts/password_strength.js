(function(){
  ...
  function PasswordStrength() {
    ...
    this.score = 0;
    this.status = null;
    // Why the password was rejected outright: "too_short", "common_word",
    // "repeated_character", "excluded_characters", or null.
    this.invalidReason = null;
    // The number of characters a password must have. Leave it null to apply no
    // hard minimum.
    this.minLength = null;
  }
  ...
  PasswordStrength.fn.test = function() {
    ...
    if (this.isTooShort()) {
      this.invalidReason = "too_short";
      this.status = "invalid";
    } else if (this.containInvalidMatches()) {
      this.invalidReason = "excluded_characters";
      this.status = "invalid";
    } else if (this.usesCommonWord()) {
      this.invalidReason = "common_word";
      this.status = "invalid";
    } else if (this.containInvalidRepetition()) {
      this.invalidReason = "repeated_character";
      this.status = "invalid";
    } else {
  ...
  PasswordStrength.fn.isTooShort = function() {
    if (!this.minLength) {
      return false;
    }

    return Array.from(this.password || "").length < this.minLength;
  };
  ...
  // Check the password against the blocklist. A password matches when it is on
  // the list as typed, when it is on the list once upper case and leet
  ...
  PasswordStrength.fn.usesCommonWord = function() {
    var list = PasswordStrength.blocklist || PasswordStrength.commonWords;
    var candidates = this.blocklistCandidates(this.password);

    for (var i = 0; i < candidates.length; i++) {
      if (list.indexOf(candidates[i]) >= 0) {
        return true;
      }
    }
  ...
  PasswordStrength.fn.blocklistCandidates = function(text) {
    var normalised = (text || "").toString();
    normalised = normalised.normalize ? normalised.normalize("NFKC") : normalised;
    normalised = normalised.toLowerCase();

    var leet = Array.from(normalised).map(function(character) {
      return LEET_SUBSTITUTIONS.hasOwnProperty(character) ? LEET_SUBSTITUTIONS[character] : character;
    }).join("");

    var candidates = [
      normalised,
      leet,
      normalised.replace(TRAILING_DIGITS_RE, ""),
      leet.replace(TRAILING_DIGITS_RE, "")
    ];
  ...
  PasswordStrength.fn.sequences = function(text) {
    ...
    var sequenceSize = 0;
    var codes = [];
    // Codepoints, not UTF-16 units: a character outside the basic plane is one
    // character to the person typing it, and counting its halves turns it into
    ...
    for (var i = 0; i < len; i++) {
      currentCode = characters[i].codePointAt(0);
  ...
  // options: {exclude: /\s/, minLength: 12}
  PasswordStrength.test = function(username, password, options) {
  ...
  // Replace the bundled list of common passwords with your own, for example the
  // breached passwords you downloaded from Have I Been Pwned. Entries are
  // compared in lower case. Set it to null to go back to the bundled list.
